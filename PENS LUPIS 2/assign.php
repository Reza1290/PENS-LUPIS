<?php 
    ...
    // -- JOIN `tb_enroll` e ON m.matkul_id = e.matkul_id -- matkul + enroll + task + class; --
    // JOIN `tb_task` t ON e.dosen_id = t.user_id
    // -- JOIN `tb_class` c ON e.class_id = c.class_id  
    // WHERE e.dosen_id = '$dosen_id' AND t.class_id='$id';
    // ");
    $matkul = mysqli_query($conn,"SELECT * FROM `tb_enroll` e JOIN `tb_matkul` m ON e.matkul_id=m.matkul_id WHERE e.dosen_id='$dosen_id' AND m.matkul_id='$mk'");
    $query = mysqli_query($conn,"SELECT * FROM `tb_task` WHERE user_id ='$dosen_id' AND class_id='$id' AND matkul_id='$mk'");
    $row = mysqli_fetch_object($matkul);
                    ...
                    <div class="child-card" style="width: 70%;">
                        <div class="card-content" >
                            <div class="card-header">
                                Mata Kuliah  <strong><?php echo "&nbsp".$row->matkul_name;?></strong>
                            </div>
                            <div class="card-body">
                                <?php 
                                $posts = [];
                                ...
                                for($i=0;$i<$c;$i++){
                                    ?>
                                    <a href="view-assign.php?id_task=<?php echo $posts[$i]['task_id'];?>" class="card-menu">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M96 0C43 0 0 43 0 96V416c0 53 43 96 96 96H384h32c17.7 0 32-14.3 32-32s-14.3-32-32-32V384c17.7 0 32-14.3 32-32V32c0-17.7-14.3-32-32-32H384 96zm0 384H352v64H96c-17.7 0-32-14.3-32-32s14.3-32 32-32zm32-240c0-8.8 7.2-16 16-16H336c8.8 0 16 7.2 16 16s-7.2 16-16 16H144c-8.8 0-16-7.2-16-16zm16 48H336c8.8 0 16 7.2 16 16s-7.2 16-16 16H144c-8.8 0-16-7.2-16-16s7.2-16 16-16z"/></svg>
                                            <div class="caption">
                                                <?php echo $posts[$i]['task_name']?> 
                                                <p><?= $posts[$i]['task_desc']?></p>
                                            </div>    
                                        </div>    
                                    </a>
                                <?php }?>
                            ...
                            <div class="card-footer">
                                <div class="wrap-foot">
                                    ...
                                    <?php include 'tambah-tugas.php' ?>
                                        <button class="openButton" onclick="openForm()"><strong>Tambah Tugas</strong></button>
...
<?php }else{ header('location: tugas.php'); }?>
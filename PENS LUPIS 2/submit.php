<?php 
    ...
    include 'db.php';
    $id = $_GET['id'];
    $mhs = $_SESSION['a_global']->user_id;
    $q = mysqli_query($conn,"SELECT * FROM `tb_enroll_class` WHERE matkul_id='$id'
    AND mahasiswa_id='$mhs'");
    $ec = mysqli_fetch_array($q);
    $class = $ec['class_id'];
    // print_r($ec);
    $task = mysqli_query($conn,"SELECT * FROM `tb_task` WHERE matkul_id='$id' AND class_id='$class'");    
                    ...
                    <div class="child-card" style="width: 70%;">
                        <div class="card-content" >
                            <div class="card-header">
                                Tugas  <strong><?php echo "&nbsp";?></strong>
                            </div>
                            <div class="card-body">
                                <?php 
                                ...
                                while($row = mysqli_fetch_array($task)){
                                    ...
                                    <a href="submit-task.php?id_task=<?php echo $row['task_id'];?>" class="card-menu">

<?php 
    ...
    if($_SESSION['a_global']->role == 1){
    include 'db.php';
    $id = $_GET['id'];

    $task_merge = mysqli_query($conn,"SELECT * FROM `tb_tugas`
    INNER JOIN `tb_submit` ON  tb_tugas.tugas_id = tb_submit.tugas_id
    INNER JOIN `tb_matkul` ON tb_tugas.matkul_id = tb_matkul.matkul_id
    -- WHERE tb_tugas.tugas_id='".$id."'
    ");
    $task = mysqli_fetch_object($task_merge);    
                    ...
                    <div class="child-card" style="width: 70%;">
                        <div class="card-content" >
                            <div class="card-header">
                                Mata Kuliah  <strong><?php echo "&nbsp".$task->matkul_name;?></strong>
                            </div>
                            <div class="card-body">
                                
                                <div class="card-menu-content">
                                    <div class="caption">
                                        <strong>Judul Tugas : </strong> <?= $task->tugas_name?>
                                        <p>Deskripsi : <?= $task->tugas_description?></p>
                                ...
                                <table class="tabel">
                                    ...
                                    <tbody>
                                        ...
                                        $merge = mysqli_query($conn,"SELECT * FROM `tb_submit`,`tb_admin` WHERE tb_submit.mahasiswa_id = tb_admin.admin_id AND tugas_id='".$id."'");
                                        // $datamhs = mysqli_fetch_array($merge);
                                        while($row = mysqli_fetch_array($merge)){
                                        ?>
                                        <tr>
                                            <td><?php echo $no++?></td>
                                            ...
                                            <td class="tabel-tombol"><a href="download-file.php?down_id=<?php echo $row['file_path']; ?>">Download</a>
...
echo '<script>window.location="tugass.php"</script>';
}
?>
